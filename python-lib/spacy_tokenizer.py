...
Token.set_extension(
    "is_symbol",
    getter=lambda token: re.sub(SYMBOL_REGEX, "", token.text) == "",
    force=True,
)
Token.set_extension(
    "is_unit",
    getter=lambda token: any([token.lower_.replace(s, "").isdigit() for s in UNITS]),
    force=True,
)
Token.set_extension(
    "is_time",
    getter=lambda token: re.sub(TIME_REGEX, "", token.lower_).isdigit(),
    force=True,
)
